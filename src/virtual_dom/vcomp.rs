...
type Generator = dyn Fn(GeneratorType) -> ComponentLifespan;
...
impl VComp {
        ...
        COMP: Component,
    {
        ...
        let generator = move |generator_type: GeneratorType| -> ComponentLifespan {
            match generator_type {
                GeneratorType::Premount(element, dummy_node) => {
                    let scope: Scope<COMP> = Scope::new();

                    let mut scope = scope.prepare_for_mount(
...
impl Unmounted {
    ...
    fn premount(self, parent: Element, dummy_node: TextNode) -> ComponentLifespan {
        (self.generator)(GeneratorType::Premoutn(parent, dummy_node))
    }
    
    /// Mount a virtual component using a generator.
    fn mount(self, parent: Element, dummy_node: TextNode) -> ComponentLifespan {
        (self.generator)(GeneratorType::Mount(parent, dummy_node))
    }

    /// Overwrite an existing virtual component using a generator.
    fn replace(self, old: Mounted) -> ComponentLifespan {
